...
const moduleRoot = path.join(root, 'src/modules/patterns');
const manifest = {};
...
const output = path.join(root, 'public/patterns-source.json');
const content = JSON.stringify(manifest, null, 2) + '\n';
if (process.argv.includes('--check')) {
  if (!fs.existsSync(output) || readSource(output) !== content) throw Error('Pattern source stale. Run npm run export:patterns.');
