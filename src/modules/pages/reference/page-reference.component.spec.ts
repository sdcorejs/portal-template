...
describe('CASE-REFERENCE — business route lifecycle', () => {
  it('guards every editable page and preserves old URLs as redirects', () => {
    const screens = pagesRoutes.filter(route => route.loadComponent);
    ...
    for (const example of PAGE_EXAMPLES) {
      ...
      expect(pagesRoutes.find(route => route.path === example.id)?.redirectTo).toBe(example.group + '/' + example.id);
    }
  });
});
