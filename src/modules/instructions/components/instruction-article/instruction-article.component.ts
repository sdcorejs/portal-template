...
SdTabComponent({
  ...
  name: args => INSTRUCTIONS.find(item => args.url?.split(/[?#]/)[0] === '/instructions/' + item.path)?.title ?? 'Instructions',
  icon: args => INSTRUCTIONS.find(item => args.url?.split(/[?#]/)[0] === '/instructions/' + item.path)?.icon ?? 'school',
